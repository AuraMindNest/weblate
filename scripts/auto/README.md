...
1. Create `web.json` in this directory:
   ```json
   {"weblate_url": "http://localhost:8080", "api_token": "wlu_YOUR_TOKEN"}
   ```
   Keep it private (gitignored).
1. Create component/project JSON config. See `quickbook-test-component-configs/setup.json` or `boost-submodule-component-configs/` for examples.
