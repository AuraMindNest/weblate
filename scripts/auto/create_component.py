...
class WeblateProjectCreator:
    ...
    def project_exists(self, slug: str) -> bool:
        ...
        try:
            self._make_request("GET", f"projects/{slug}/", silent_on_404=True)
            return True
        except requests.exceptions.HTTPError as e:
    ...
    def create_project(self, project_config: dict[str, Any]) -> dict[str, Any]:
        ...
        response = self._make_request("POST", "projects/", data=project_config)
    ...
    def create_component(
        self, project_slug: str, component_config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        response = self._make_request(
            "POST", f"projects/{project_slug}/components/", data=component_config
        )
        component = response.json()
        print(f"[SUCCESS] Component created: {component['web_url']}", flush=True)
        return component

    def trigger_component_update(self, project_slug: str, component_slug: str) -> bool:
        """Trigger VCS update for a component."""
        print(
            f"[INFO] Triggering VCS update for {project_slug}/{component_slug}",
            flush=True,
        )
    ...
    def verify_and_fix_component_settings(
        self, project_slug: str, component_slug: str, expected_config: dict[str, Any]
    ) -> bool:
        ...
        try:
            current_config = self._get_component_config(project_slug, component_slug)
            updates_needed = self._find_config_differences(
                current_config, expected_config
            )

            if updates_needed:
                self._apply_config_updates(project_slug, component_slug, updates_needed)
            else:
    ...
    def _find_config_differences(
        self, current_config: dict[str, Any], expected_config: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        for field in critical_fields:
            ...
            expected = self._normalize_value(expected_config[field], field)
            current = self._normalize_value(current_config.get(field), field)
    ...
    def _apply_config_updates(
        self, project_slug: str, component_slug: str, updates: dict[str, Any]
    ) -> None:
    ...
    def wait_for_component_ready(
        self, project_slug: str, component_slug: str, max_wait: int = DEFAULT_WAIT_TIME
    ) -> bool:
        ...
        print(
            f"[INFO] Timeout in {max_wait}s. Checking every {POLL_INTERVAL}s...",
            flush=True,
        )
        ...
        while time.time() - start_time < max_wait:
            ...
            if self._check_component_ready(project_slug, component_slug, silent=True):
                return True
            time.sleep(POLL_INTERVAL)

        elapsed = int(time.time() - start_time)
        print(
            f"[INFO] Timeout after {elapsed}s (normal for large repositories)",
            flush=True,
        )
        print(
            "[INFO] Component created successfully, tasks continuing in background",
            flush=True,
        )
        return False

    def _check_component_ready(
        self, project_slug: str, component_slug: str, silent: bool = False
    ) -> bool:
        ...
        try:
            ...
            if locked:
                if not silent:
                    print(
                        "[WARNING] Component is locked (repository error)", flush=True
                    )
                return False
    ...
    def get_statistics(self, project_slug: str, component_slug: str) -> dict[str, Any]:
...
def merge_configs(
    web_config: dict[str, Any], component_config: dict[str, Any]
) -> dict[str, Any]:
...
def create_example_config_file(filename: str = "weblate_config.example.json") -> None:
...
def print_success_summary(project: dict[str, Any], component: dict[str, Any]) -> None:
...
Note:
  ...
  For automatic web.json loading, use create_component_and_add_translation.py instead.
    ...
    parser.add_argument("--config", type=str, help="Component configuration JSON file")
    parser.add_argument(
        "--web-config",
        type=str,
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="Interactive configuration mode"
    )
    parser.add_argument(
        "--example", action="store_true", help="Create example configuration file"
    )
